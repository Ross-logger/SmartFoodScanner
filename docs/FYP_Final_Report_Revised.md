...
Evaluation was conducted on one hundred test images with ground truth aligned to the augmented dataset. For the model-based local pipeline, the mean ingredient precision was 97.75 percent, the mean ingredient recall was 95.60 percent, and the mean F1 score was 96.54 percent. For the LLM-based cloud pipeline, consisting of Mistral OCR followed by LLM extraction, the merge-level F1 score was 95.17 percent, evaluated against a stricter ground-truth format that preserves compound ingredient structures. End-to-end latency testing on a MacBook Pro M1 Pro recorded a median total time of 8.26 seconds for the model-based flow and 18.35 seconds for the LLM-based flow. Unit tests achieved near-complete statement coverage across all core services, while integration tests validated the full processing pipeline for both OCR-based and barcode-based inputs.
...
#### 4.4.2 Description of Datasets and Evaluation Artefacts

All dataset files referenced in this project live in the repository under predictable paths so that training, evaluation, and automated tests can be reproduced. Table 4.1 summarises the main artefacts; the narrative below explains their role and how they relate to the methodology in Section 4.3.3.

Label photographs used in tests and in the reported accuracy study are stored under `tests/data/images/` in the project tree (approximately one hundred primary images, with additional files where augmented or variant filenames are required by the training pipeline). The original hundred photographs were captured in retail settings or taken from the Kaggle “Ingredients Image from Food Label” collection, as noted in Section 4.3.3.

**Box-level classifier data.** The ingredient versus non-ingredient classifier is trained from CSV exports of EasyOCR detections. File `training/datasets/dataset.csv` contains the baseline set: each row is one detection box with fields `image_id`, `box_id`, `text`, `confidence`, axis-aligned coordinates (`x1`, `y1`, `x2`, `y2`), and binary `label`. File `training/datasets/dataset_augmented_1000.csv` follows the same schema for the augmented corpus (on the order of ten thousand annotated boxes across roughly one thousand distinct `image_id` values), matching the “one thousand augmented samples” design described in Section 4.3.3.

**Flat ground truth (model-based pipeline).** JSON files pair each image filename with a list `true_ingredients` of strings in **flat atomic** form—compound phrases are split so that each token aligns with the delimiter-split output of the local merge-and-correction pipeline. File `training/datasets/true_ingredients.json` contains one hundred such records for the core image set. File `training/datasets/true_ingredients_augmented_1000.json` contains one thousand records in the same schema, aligned with the augmented image identifiers; it supports large-scale offline scoring (for example via `training/evaluate_merge_predictions.py`) and documents the labelling convention used when exporting classifier predictions to `outputs/model_predictions.csv`. Other variants in `training/datasets/` (for example `true_ingredients_augmented_600.json`, intermediate atomic clean-up passes) are retained for ablations and development history but are not required to interpret the main results in Chapter 5.

**Nested ground truth (LLM pipeline).** File `tests/data/true_ingredients_for_llm.json` contains one hundred records for the same evaluation photographs as the headline accuracy figures in Chapter 5. Here `true_ingredients` preserves **compound structure** and fuller additive wording (for example a single entry “Permitted Stabilizer (E 460(i))” where the flat file might list only a normalised code), which matches the LLM extraction prompt and the merge-level metrics reported in Section 5.2.2.

**Caches and test fixtures.** Directory `tests/data/` also holds cached OCR transcripts, comparison outputs, and performance logs used by integration and performance tests so that CI runs do not depend on live API calls. These files are secondary to the curated ground-truth JSON above.

**External data mixed into augmentation.** Text statistics from the Open Food Facts ingredient-detection dataset (Bournhonesque, 2023) were used when synthesising additional box content during augmentation, as described in Section 4.3.3; they do not appear as a separate downloadable table in the repository but are cited here for provenance.

**Table 4.1.** Main dataset and ground-truth files (paths relative to the project root).

| Artefact | Location (relative path) | Role |
| -------- | ------------------------ | ---- |
| Label images | `tests/data/images/` | Input photographs for OCR, classifier training/eval, and tests |
| Baseline box labels | `training/datasets/dataset.csv` | Per-box text, geometry, confidence, and ingredient/non-ingredient label |
| Augmented box labels | `training/datasets/dataset_augmented_1000.csv` | Augmented box-level training set (~1k image IDs) |
| Flat ingredient ground truth (100) | `training/datasets/true_ingredients.json` | Atomic ingredient lists for the core image set |
| Flat ingredient ground truth (1k) | `training/datasets/true_ingredients_augmented_1000.json` | Atomic lists aligned with augmented IDs; merge evaluation input |
| Nested LLM ground truth | `tests/data/true_ingredients_for_llm.json` | Compound-preserving lists for LLM pipeline metrics (100 images) |

The quantitative extraction results in Chapter 5 are reported as means over **one hundred** test images; the flat versus nested JSON pair above is what distinguishes scoring for the model-based pipeline from scoring for the LLM-based pipeline. Full paths on disk are obtained by joining these relative paths with the repository root of the Smart Ingredients Scanner source tree.

#### 4.4.3 Unit Test Design
...
Ingredient extraction accuracy is evaluated separately for each processing flow using ground-truth ingredient lists; file names, formats, and directory layout are specified in Section 4.4.2. For the model-based pipeline, merged predictions can be scored offline with the training script `training/evaluate_merge_predictions.py`, which compares exported classifier outputs (for example `outputs/model_predictions.csv`) against a ground-truth JSON file (for example `training/datasets/true_ingredients_augmented_1000.json`) using RapidFuzz-based matching at a configurable fuzzy threshold, yielding per-image ingredient precision, recall, and F1 together with wall-clock timing. For the LLM-based pipeline, the nested reference lists in `tests/data/true_ingredients_for_llm.json` support merge-level or token-level metrics. Additional scripts may compute fuzzy token matching at threshold 0.8 and merge-based containment metrics for sensitivity analysis. Per-category metrics for dietary restriction detection are also computed to identify areas that may require improvement. Numerical results of these evaluations are reported in the results chapter rather than here.
...
This chapter presents the quantitative results of the evaluation conducted on the Smart Ingredients Scanner. Two categories of evaluation are reported. The first is ingredient extraction accuracy, which measures how well each processing flow recovers individual ingredients from food-label images relative to manually curated ground-truth lists, together with a negative case analysis that interprets residual errors and relates them to concrete failure modes in each stage of the pipelines (Section 5.2.3). The second is end-to-end processing latency, which measures the wall-clock time from image upload through to dietary analysis result for each flow under realistic conditions. The evaluation methodology, dataset layout, and metric definitions used to produce these results were described in Sections 4.4.2 and 4.4.5. A summary of the automated test suite that validates correctness of individual components and integration paths is also provided.

### 5.2 Ingredient Extraction Accuracy

#### 5.2.1 Model-Based Pipeline

The model-based pipeline, consisting of EasyOCR text detection, the Logistic Regression box classifier, the spatial merge algorithm, and the OCR corrector described in Sections 4.3.2 through 4.3.7, was evaluated on one hundred test images drawn from the same image pool used to construct the augmented training dataset described in Section 4.3.3. Each ground-truth entry pairs an image filename with a manually verified list of ingredient strings, where compound ingredients are split into their individual sub-components to match the flat output format of the model-based pipeline.

The evaluation script runs the full extraction pipeline on box-level classifier predictions, applies OCR correction, and compares the resulting merged ingredient text against the ground-truth lists using RapidFuzz similarity scoring. For each ground-truth ingredient, the script computes the maximum similarity score across four complementary measures and counts the ingredient as matched if the best score meets or exceeds a configurable fuzzy threshold. Precision is the proportion of predicted segments that match at least one ground-truth ingredient, and recall is the proportion of ground-truth ingredients found within the merged text. The F1 score is the harmonic mean of precision and recall, computed per image and then averaged across the test set.
...
The LLM-based pipeline, consisting of Mistral OCR for text extraction followed by Mistral 7B for structured ingredient extraction as described in Sections 4.3.1 and 4.3.4, was evaluated on the same one hundred test images. A separate ground-truth file was prepared for this evaluation in which compound ingredients are preserved in their nested form rather than split into individual sub-components. For example, where the model-based ground truth lists "sugar", "blueberry puree", "water", "pectin (INS 440)", and "lemon juice concentrate" as five separate entries, the LLM ground truth retains the single entry "processed blueberry pulp (sugar, blueberry puree, water, pectin (INS 440), lemon juice concentrate)". This format was chosen deliberately because the LLM is instructed to preserve compound structures in its output, and presenting ingredients with their hierarchical relationships intact provides richer context for the user when reviewing scan results. A user who sees a parent ingredient alongside its sub-ingredients can make a more informed dietary judgement than one who sees only a flat, unordered list. The consequence of this design choice is that the LLM evaluation is inherently stricter, as the predicted text must match longer and more structurally complex reference strings.

The evaluation compares the LLM output against the ground truth using three matching strategies: exact case-normalised string equality, fuzzy matching at a similarity threshold of 0.8, and merge-level containment matching. Under merge-level matching, the LLM-based pipeline achieved a mean F1 of 95.17 percent over the one hundred test images. Under fuzzy matching, the F1 rose to 95.75 percent, indicating that the language model corrects most OCR-related misspellings successfully but occasionally deviates from the expected output format or introduces minor hallucinations. For instance, the model may return an additive code such as "E 460(i)" in isolation rather than the full contextual form "Permitted Stabilizer (E 460(i))" present in the ground truth. Although these represent the same ingredient and would produce identical dietary analysis outcomes, the evaluation penalises such mismatches because the string forms differ. The reported F1 of 95.17 percent is therefore a conservative lower bound on the pipeline's true extraction effectiveness.

This hallucination behaviour is consistent with findings in the wider literature on smaller language models. Islam, Lauscher, and Glavas (2025) conducted a large-scale study of hallucination rates across thirty languages and six open-source LLM families and found that smaller models hallucinate significantly more than their larger counterparts. Mistral 7B, as a seven-billion-parameter model, falls squarely within the size range where elevated hallucination rates are expected. Despite this, a merge-level F1 above 95 percent represents a strong result, particularly given the strict evaluation format and the model's ability to handle semantically complex label text. The primary trade-off relative to the model-based pipeline is latency, which is examined in the next section.

#### 5.2.3 Negative Case Analysis

High aggregate F1 scores leave a small but non-negligible tail of failures. This subsection interprets the error rates implied by Sections 5.2.1 and 5.2.2 and maps them to likely causes at each pipeline stage, so that the limitations of the system are explicit alongside the headline metrics.

**Model-based pipeline.** Under exact matching, mean precision of 97.75 percent corresponds to approximately two in every hundred predicted ingredient segments that do not align with any ground-truth ingredient (false positives). Mean recall of 95.60 percent corresponds to approximately four in every hundred ground-truth ingredients that are not recovered (false negatives). When the fuzzy threshold is relaxed to 0.8, the F1 score increases only from 96.54 to 97.10 percent. The small gap between exact and fuzzy outcomes indicates that minor formatting or orthographic differences account for only a fraction of the total error mass; the majority of mismatches involve missing or extraneous text, or lexical forms that remain below the fuzzy threshold, rather than a single stray comma or one-character OCR slip.

False positives arise when a segment is emitted as an ingredient although it is not part of the true list. In this architecture, the dominant mechanism is the Logistic Regression box classifier accepting a box that actually carries marketing copy, brand names, decorative slogans, or section headings visually similar to ingredient lines. EasyOCR can also fragment a line into short tokens; isolated fragments may resemble legitimate ingredient tokens and pass classification even though they are not meaningful list entries in context.

False negatives stem from several independent failure points. If EasyOCR does not produce a detection for a region—because of low contrast, glare, motion blur, very small type, or strong curvature on cylindrical packaging—no later stage can reconstruct that ingredient. If a valid list fragment is detected but classified as non-ingredient, it is discarded before merging. The spatial merge algorithm may fail to group boxes that belong to one logical line when column layout or vertical spacing is irregular, yielding incomplete strings that no longer match the reference. Finally, the OCR corrector is constrained by its vocabulary and correction strategies: specialised chemical names, rare additives, or non-English synonyms that lie outside the dictionary may remain in a form that fails exact or high-threshold fuzzy matching even when the raw OCR is visually plausible. These mechanisms align with the qualitative patterns noted in Section 5.2.1, namely curved or distorted text and multi-language labels.

**LLM-based pipeline.** The error profile differs in emphasis. Mistral OCR can omit or misread regions in the same physical situations as any OCR engine, but many of the penalties observed under merge-level matching reflect structural disagreement with the ground truth rather than raw character errors: compounds may be flattened or nested differently, parenthetical qualifiers may be shortened, or additive codes may appear in abbreviated form (for example "E 460(i)" instead of a longer permitted-stabiliser string), as described in Section 5.2.2. Fuzzy matching absorbs part of this gap; the remainder reflects format strictness in evaluation. Occasional hallucinated list items contribute false positives under string-based metrics even when a rule-based dietary check might still reach the same safety conclusion if synonyms were normalised.

**Implications for use.** The negative cases above motivate two practical points already supported by the application design. First, users with strict medical or religious requirements should treat automated extraction as advisory and use manual correction when a line appears incomplete or inconsistent with the packaging. Second, flow choice trades off failure modes: the LLM-based path is better suited to dense, parenthesis-heavy, or semantically tangled English labels at the cost of latency and external dependencies, whereas the model-based path favours speed and offline operation when the label is clear and predominantly English.

### 5.3 Processing Latency

End-to-end latency was measured by running both processing flows on ten representative test images through the full application pipeline, from image upload and OCR through ingredient extraction to LLM-based dietary analysis. The test was executed on a MacBook Pro 16-inch with an Apple M1 Pro processor and 16 GB of RAM.
...
The OCR stage of the model-based flow, which uses EasyOCR, averaged 4.87 seconds per image, approximately 2.3 times slower than Mistral OCR at 2.12 seconds. This difference is attributable to the test environment rather than algorithmic capability. All services, including EasyOCR, the LLM extraction server, and the LLM dietary analysis service, ran concurrently on the same MacBook Pro M1 Pro, competing for limited CPU and memory resources. The M1 Pro lacks CUDA-compatible GPU acceleration, forcing EasyOCR to fall back to CPU execution. Mistral OCR, by contrast, is a cloud-hosted API whose inference executes on dedicated GPU servers, returning results over the network with comparatively low round-trip overhead. To contextualise the EasyOCR figure, Nagayi et al. (2025) evaluated EasyOCR on food packaging images of comparable complexity and report an average processing time of 0.81 seconds per image when running on an NVIDIA GeForce RTX 3070 GPU. This confirms that the 4.87-second figure observed in this project reflects the constrained test environment rather than a limitation of the EasyOCR engine itself. Substituting the GPU-accelerated OCR time of 0.81 seconds into the model-based pipeline while holding the extraction and analysis stages constant yields a projected average total of approximately 7.1 seconds (0.81 plus 0.22 plus 6.04), which would make the model-based flow roughly 2.7 times faster than the LLM-based flow at 19.02 seconds.

The extraction stage accounts for the largest difference between the two flows. The model-based extraction, comprising box classification, spatial merging, and OCR correction, completed in an average of 0.22 seconds because all operations run locally using pre-loaded models and a pre-built dictionary. The LLM-based extraction averaged 10.85 seconds because it requires a full inference round-trip to the language model server. The analysis stage, which performs LLM-based dietary evaluation, took approximately 6.0 seconds in both flows because both use the same LLM analysis service regardless of how ingredients were extracted. This analysis was also served by a local LLM instance running on the same M1 Pro hardware; a cloud-hosted or GPU-accelerated LLM would reduce this figure further, benefiting both flows equally.
...
The automated test suite is organised into three tiers: unit, integration, and performance. Unit tests cover individual components in isolation. The rule-based dietary analysis engine is validated across all supported restriction categories, including halal, gluten-free, vegetarian, vegan, nut-free, and dairy-free, with both positive and negative test cases. The LLM-based dietary analysis module is tested for prompt construction, response validation, and fallback behaviour when the LLM service is unavailable. The ingredient extraction service is tested for LLM extraction helpers, format handling, and edge cases such as empty input and percentage removal. The OCR service is validated for confidence filtering, text extraction, image format support for JPEG, PNG, and HEIF, and error handling for corrupted images. The barcode lookup flow is tested for Open Food Facts API response parsing, ingredient extraction, allergen formatting, and error handling for missing products. The confidence-based filtering logic that determines which OCR segments should bypass SymSpell correction is validated separately.

Integration tests validate interactions between components. The full-pipeline test exercises end-to-end flows from OCR text through extraction and analysis for both halal and gluten-free scenarios, including accuracy-style test cases evaluated with a dedicated evaluation metrics utility. The dietary profiles test validates the ingredient analysis function across multiple profile types using synthetic OCR sample data with mocked LLM services. The API endpoints test uses the FastAPI test client to validate all REST endpoints, including authentication, dietary profile management, scan submission, and history retrieval, against an in-memory SQLite database.

A dedicated performance test measures real, non-mocked response times for OCR processing, barcode lookup, ingredient extraction, and dietary analysis under realistic conditions. The latency results reported in Section 5.3 were produced by this test suite.

Test isolation is maintained through shared fixtures that provide an in-memory SQLite database, test user accounts with hashed passwords, pre-configured dietary profiles for common restriction combinations, and mock services that replace external dependencies during unit and integration testing. Statement coverage across all core backend services was measured using Coverage.py. Figure 7 presents the coverage report, showing that all core service modules achieved statement coverage of 94 percent or above.
...
Ahmed, S., Elahi, M., & Ricci, F. (2018). A conversational recommender system for personalized food suggestions. *Information Systems*, *72*, 95–109. [https://doi.org/10.1016/j.is.2017.10.003](https://doi.org/10.1016/j.is.2017.10.003)

Assiri, F. Y., Almulhim, M. F., Aldayel, T. S., Alabdulqader, N. A., Almashouq, M. K., & Alrubaish, A. M. (2025). Extract nutritional information from bilingual food labels using OCR and machine learning. *Frontiers in Nutrition*. [https://www.ncbi.nlm.nih.gov/pmc/articles/PMC10658186/](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC10658186/)

Baker, E., Foubister, C., Shucksmith, J., Akhter, N., Battista, K., Buxton, D., Fairbrother, H., Hind, D., Hock, E., Jones, G., Limb, E., Machaczek, K. K., McGowan, L., Parretti, H. M., Quirk, H., Sherar, L. B., Wilkinson, S., Zizzo, G., & Relton, C. (2025). The impact of the Change4Life Food Scanner on children's diets and parental psychological outcomes: A randomised pilot and feasibility study. *BMC Public Health*, *25*(1), Article 340. [https://doi.org/10.1186/s12889-025-23400-0](https://doi.org/10.1186/s12889-025-23400-0)

Bournhonesque, R. (2023). *Open Food Facts ingredient-detection dataset* (Version 1.1) [Dataset]. Hugging Face. [https://huggingface.co/datasets/openfoodfacts/ingredient-detection/tree/v1.1](https://huggingface.co/datasets/openfoodfacts/ingredient-detection/tree/v1.1)

Chouraqui, J.-P., Turck, D., Briend, A., Darmaun, D., Bocquet, A., Feillet, F., Frelut, M.-L., Girardet, J.-P., Guimber, D., Hankard, R., Lapillonne, A., Peretti, N., Roze, J.-C., Siméoni, U., Dupont, C., & the Committee on Nutrition of the French Society of Pediatrics. (2021). Religious dietary rules and their potential nutritional and health consequences. *International Journal of Epidemiology*, *50*(1), 12–26. [https://doi.org/10.1093/ije/dyaa182](https://doi.org/10.1093/ije/dyaa182)

Food Allergy Research & Education. (2020). *The food allergy consumer journey*. [https://www.foodallergy.org/sites/default/files/2020-07/FARE-Food-Allergy-Consumer-Journey-Study-July2020.pdf](https://www.foodallergy.org/sites/default/files/2020-07/FARE-Food-Allergy-Consumer-Journey-Study-July2020.pdf)

Fujisawa, H. (2008). Forty years of research in character and document recognition—An industrial perspective. *Pattern Recognition*, *41*(8), 2435–2446. [https://doi.org/10.1016/j.patcog.2008.03.015](https://doi.org/10.1016/j.patcog.2008.03.015)

Guo, Q., Tu, D., Li, G., & Lei, J. (2016). Memory matters: Convolutional recurrent neural network for scene text recognition. *arXiv:1601.01100*. [https://doi.org/10.48550/arXiv.1601.01100](https://doi.org/10.48550/arXiv.1601.01100)

IBM. (2024, April 18). What is optical character recognition (OCR)? [https://www.ibm.com/think/topics/optical-character-recognition](https://www.ibm.com/think/topics/optical-character-recognition)
...
Khamesian,model S., Arefeen, A., Carpenter, M., & Ghasemzadeh, H. (2025). NutriGen: Personalized meal plan generator leveraging large language models to enhance dietary and nutritional adherence. *arXiv:2502.20601*. [https://arxiv.org/abs/2502.20601](https://arxiv.org/abs/2502.20601)

Leba, E., Faigao, J. A., Magante, M. V., Tamba, V. J. Y., & Bingcang, I. N. E. (2024). Allertify: Android-based mobile application for food allergens identification using barcode scanning and image recognition. *Proceedings of the 11th International Scholars Conference*. [https://doi.org/10.35974/isc.v11i5.3669](https://doi.org/10.35974/isc.v11i5.3669)

Li, C., Zhang, C., Cai, Z., & Wang, C. (2021). TrOCR: Transformer-based optical character recognition. *arXiv:2109.10282*. [https://doi.org/10.48550/arXiv.2109.10282](https://doi.org/10.48550/arXiv.2109.10282)

Müller-Pérez, J., Acevedo-Duque, A., García-Salirrosas, E. E., Escobar-Farfán, M., Esponda-Pérez, J. A., Cachicatari-Vargas, E., Álvarez-Becerra, R., & Alcina De Fortoul, S. (2025). Factors influencing healthy product consumer behavior: An integrated model of purchase intention. *Frontiers in Public Health*, *13*, Article 1576427. [https://doi.org/10.3389/fpubh.2025.1576427](https://doi.org/10.3389/fpubh.2025.1576427)

Nagayi, M., Khan, A. S., Frank, T., Swart, R., & Nyirenda, C. (2025). Evaluating OCR performance on food packaging labels in South Africa. *arXiv:2510.03570*. [https://arxiv.org/abs/2510.03570](https://arxiv.org/abs/2510.03570)

Nossair, A., & El Housni, H. (2024). Eating smart: Advancing health informatics with the Grounding DINO-based dietary assistant app. *arXiv:2406.00848*. [https://arxiv.org/abs/2406.00848](https://arxiv.org/abs/2406.00848)

ODSC. (2025). The best lightweight LLMs of 2025: Efficiency meets performance. *Open Data Science Conference*. [https://odsc.medium.com/the-best-lightweight-llms-of-2025-efficiency-meets-performance-78534ce45ccc](https://odsc.medium.com/the-best-lightweight-llms-of-2025-efficiency-meets-performance-78534ce45ccc)

Salehudin, M. A. M., Basah, S. N., Yazid, H., Basaruddin, K. S., Safar, M. J. A., Som, M. H. M., & Sidek, K. A. (2023). Analysis of optical character recognition using EasyOCR under image degradation. *Journal of Physics: Conference Series*, *2641*(1), 012001. [https://doi.org/10.1088/1742-6596/2641/1/012001](https://doi.org/10.1088/1742-6596/2641/1/012001)

Shensivam. (n.d.). *Ingredients Image from Food Label* [Dataset]. Kaggle. [https://www.kaggle.com/datasets/shensivam/ingredients-image-from-food-label](https://www.kaggle.com/datasets/shensivam/ingredients-image-from-food-label)

Shepherd, R., & Raats, M. M. (Eds.). (2006). *The psychology of food choice*. CABI Publishing.

Sinha, R., & Rekha, B. S. (2025). Digitization of document and information extraction using OCR and large language models. *arXiv:2506.11156*. [https://arxiv.org/abs/2506.11156](https://arxiv.org/abs/2506.11156)

Ueno, L. (2024, March 16). Best OCR models for text recognition in images. *Roboflow Blog*. [https://blog.roboflow.com/best-ocr-models-text-recognition/](https://blog.roboflow.com/best-ocr-models-text-recognition/)

Ultra Processed Food List. (2026). *Analysis of 1.97 million food products*. [https://www.ultraprocessedfoodlist.com/](https://www.ultraprocessedfoodlist.com/)

van der Avoort, C. M., van Lee, L., van Loon, M. R., van Loo, E. J., & Feskens, E. J. (2021). Food identification by barcode scanning in the Netherlands: A quality assessment of labelled food product databases underlying popular nutrition applications. *Public Health Nutrition*, *24*(12), 4000–4007. [https://doi.org/10.1017/S1368980021002000](https://doi.org/10.1017/S1368980021002000)

Yamashita, R., Nishio, M., Do, R. K., & Togashi, K. (2018). Convolutional neural networks: An overview and application in radiology. *Insights into Imaging*, *9*(4), 611–629. [https://doi.org/10.1007/s13244-018-0639-9](https://doi.org/10.1007/s13244-018-0639-9)