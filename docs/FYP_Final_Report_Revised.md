...
Dataset construction, augmentation, vocabulary expansion, and provenance of the training images are described in Section 4.3.3. This subsection covers only how data are represented for classifier training tables, extraction evaluation, and automated tests, without naming specific files in the repository.
...
**Ancillary test data.** Automated tests may rely on cached OCR transcripts, API snapshots, and timing logs so that runs are reproducible without repeated live calls. These are supporting artefacts rather than part of the curated labelling corpus described in Section 4.3.3.
