...
This subsection summarises what data were collected, how they were expanded, and how they are represented, without tying the narrative to specific filenames in the source tree, since those may change as the project evolves. Construction rationale and vocabulary sources are described further in Section 4.3.3.

**Initial image corpus.** The starting set comprised one hundred photographs of food product labels. Images came from two sources: pictures taken in retail settings of real packaging, and images selected from the public Kaggle dataset “Ingredients Image from Food Label” (Shensivam, n.d.). Together they provide a mix of lighting conditions, fonts, and layouts typical of packaged goods sold in English-speaking markets.

**Augmentation.** To obtain a larger training set for the box classifier, the labelled corpus was expanded to on the order of one thousand **logical samples** (distinct image identifiers) using synthetic perturbations: variation of bounding-box coordinates, injection of noise into OCR confidence scores, and incorporation of frequent ingredient-like text patterns derived from the Open Food Facts ingredient-detection dataset (Bournhonesque, 2023), as detailed in Section 4.3.3. Augmented samples reuse the same per-box labelling scheme as the original images so that the classifier still learns to separate ingredient list text from surrounding packaging text.

**Box-level training table (tabular format).** Classifier training data are stored as tabular exports of EasyOCR output with manual labels. Each **row** corresponds to one text detection box. Typical **fields** are: an image identifier; a box index within that image; the recognised **text** string; the detector **confidence**; axis-aligned bounding-box coordinates (for example top-left and bottom-right in pixels); and a binary **label** indicating whether that box should be treated as part of the ingredient list versus other label content (nutrition tables, storage text, marketing lines, and so on).

**Ground truth for extraction metrics (structured lists).** Ingredient reference lists for evaluation are stored as structured records (for example JSON), each linking an image identifier to an array of **true ingredient strings**. Two conventions are used. For the **model-based pipeline**, strings are kept in **flat atomic** form: compound ingredients are split into separate entries so that scoring matches the pipeline’s comma-separated output. For the **LLM-based pipeline**, a parallel reference uses **nested wording** where appropriate (for example a single entry that preserves a full additive phrase), which aligns with the LLM’s instructed output shape and with merge-level scoring described in Chapter 5. The headline accuracy figures in Chapter 5 are means over **one hundred** test images; the distinction between flat and nested reference lists is what makes the two pipelines comparable yet fairly scored under their respective output formats.

**Ancillary test data.** Automated tests may rely on cached OCR transcripts, API snapshots, and timing logs so that runs are reproducible without repeated live calls. These are supporting artefacts rather than part of the curated labelling corpus above.
...
Ingredient extraction accuracy is evaluated separately for each processing flow using the ground-truth conventions described in Section 4.4.2. For the model-based pipeline, an offline evaluation procedure compares exported classifier-based predictions to the flat atomic reference lists using RapidFuzz-based matching at a configurable fuzzy threshold, yielding per-image ingredient precision, recall, and F1 together with wall-clock timing. For the LLM-based pipeline, the same broad procedure applies using the nested reference lists so that merge-level or token-level metrics can be computed. Additional sensitivity analyses may use relaxed fuzzy thresholds (for example 0.8) and merge-based containment rules. Per-category metrics for dietary restriction detection are also computed to identify areas that may require improvement. Numerical results of these evaluations are reported in the results chapter rather than here.
