...
**Empirical evaluation (box classifier and merge pipeline).** Test-set predictions were exported to outputs/model_predictions.csv and evaluated with training/evaluate_merge_predictions.py against datasets/true_ingredients_augmented_1000.json using fuzzy-match threshold 100. Over one hundred images, mean ingredient precision was 0.9775, mean ingredient recall 0.9560, mean ingredient F1 0.9654, and mean wall-clock time approximately 0.12 seconds per image.
...
Ingredient extraction accuracy is evaluated separately for each processing flow using ground-truth ingredient lists. For the model-based pipeline, the training evaluation script evaluate_merge_predictions.py scores merged text from classifier predictions (outputs/model_predictions.csv) against datasets/true_ingredients_augmented_1000.json using RapidFuzz-based matching at fuzzy threshold 100, reporting per-image ingredient precision, recall, and F1 aggregated over one hundred images. Under this protocol, mean ingredient precision was 97.75 percent, mean ingredient recall 95.60 percent, mean ingredient F1 96.54 percent, and mean wall-clock time approximately 0.12 seconds per image. For the LLM-based pipeline, merge-level evaluation on the same one hundred images reported precision 95.18 percent, recall 95.19 percent, F1 95.17 percent, and mean wall-clock time approximately 6.9 seconds per image. Additional evaluations may compute fuzzy token matching at threshold 0.8 and merge-based containment metrics for comparison with earlier benchmarks. Per-category metrics for dietary restriction detection are also calculated to identify specific areas that may require improvement.
...
The solution was the dual-flow architecture described in Section 4.1. The model-based pipeline, combining the box classifier, merge module, and OCR corrector, was developed as a fast alternative that runs locally with locally loaded models and a pre-built food ingredient dictionary, avoiding LLM and optional Mistral OCR API calls. Offline evaluation of the merge stage on one hundred test images averaged approximately 0.12 seconds per image, compared with approximately 6.9 seconds per image for the LLM-based extraction flow on the same set. Users who prefer Mistral OCR and LLM-driven parsing for difficult label layouts can enable the LLM-based flow through their profile settings, accepting the latency trade-off. The model-based pipeline serves as the default processing mode when enabled in configuration.
...
This chapter has presented the technical implementation of the Smart Ingredients Scanner, covering the dual-flow processing architecture, database design, algorithm implementations, testing methodology, and problems encountered during development. The dual-flow design offers users a choice between the LLM-based pipeline (Mistral OCR and Mistral 7B extraction), evaluated at merge-level F1 95.17 percent with mean time approximately 6.9 seconds per image over one hundred test images, and the model-based pipeline (EasyOCR, Logistic Regression box classifier, merge, and OCR corrector), evaluated at mean ingredient F1 96.54 percent with mean time approximately 0.12 seconds per image under the same evaluation protocol described in Section 4.4.4. The problems encountered during development and their solutions, including the migration from PyTesseract to EasyOCR, the design of the dual-flow architecture to address LLM latency, and the construction of a custom training dataset, illustrate the practical engineering decisions involved in deploying an AI-powered application in a real-world context.
