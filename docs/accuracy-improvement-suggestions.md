...
## Evaluation Methodology: How Results Are Compared

For each entry, we compare **extracted_ingredients** (pipeline output) with **true_ingredients** (ground truth). Both are treated as sets of strings after normalisation (lowercase, strip whitespace).

### Exact Match

Two ingredients match only if they are identical after normalisation. Metrics:

- **Precision** = correct predictions / total predictions (how many extracted items are correct)
- **Recall** = correct predictions / total ground truth (how many ground-truth items were found)
- **F1** = harmonic mean of precision and recall

```python
def exact_match(predicted: list[str], ground_truth: list[str]) -> tuple[float, float, float]:
    pred_set = set(x.lower().strip() for x in predicted)
    gt_set = set(x.lower().strip() for x in ground_truth)
...
### Fuzzy Match (0.85 threshold)

For each predicted ingredient, we find the best-matching ground-truth item using `difflib.SequenceMatcher`. A match counts if similarity ≥ 0.85. This allows minor OCR/spelling differences (e.g. "cocoa" vs "cococa") to still count as correct.

```python
from difflib import SequenceMatcher

def fuzzy_match(predicted: list[str], ground_truth: list[str], threshold: float = 0.85) -> tuple[float, float, float]:
    if not ground_truth:
    ...
    for p in predicted:
        ...
        for idx, g in enumerate(ground_truth):
            if idx in matched_gt:
                continue
            score = SequenceMatcher(None, pl, g.lower().strip()).ratio()
            if score > best_score:
...
The reported metrics are the **mean** of precision, recall, and F1 across all 75 entries. Each entry is evaluated independently; there is no partial credit for near-matches in exact mode.
